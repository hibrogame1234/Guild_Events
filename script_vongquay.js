// 1. CẤU HÌNH BIẾN TOÀN CỤC (Mặc định mở ngày 30 hàng tháng)
let WHEEL_CONFIG = { dayOfMonth: 30, startHour: 20, startMinute: 0, endHour: 21, endMinute: 0 };

// Lắng nghe cấu hình từ Firebase (Nhánh 'eventConfig/wheel')
    ...
    if (window.db) {
        window.dbOnValue(window.dbRef(window.db, 'eventConfig/wheel'), (snapshot) => {
            ...
            if (data) {
                ...
                // Điền dữ liệu vào Admin Modal nếu đang mở
                if(document.getElementById('edit-date-month')) {
                    document.getElementById('edit-date-month').value = data.dayOfMonth || 30;
...
// 2. HÀM KIỂM TRA TRẠNG THÁI MỞ CỬA (Theo ngày trong tháng)
function isWheelOpen() {
    if (!WHEEL_CONFIG) return false;

    const now = new Date();
    const currentDate = now.getDate(); // Lấy ngày hiện tại (1-31)
    const currentHour = now.getHours();
    const currentMinute = now.getMinutes();

    // Kiểm tra đúng ngày trong tháng
    if (currentDate !== (WHEEL_CONFIG.dayOfMonth || WHEEL_CONFIG.dayOfWeek)) return false; 
...
if (saveBtn) {
    saveBtn.addEventListener('click', () => {
        ...
        window.dbSet(window.dbRef(window.db, 'eventConfig/wheel'), newConfig)
            .then(() => {
                alert("Đã cập nhật giờ sự kiện tháng!");
