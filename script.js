...
if (window.db) {
    window.dbOnValue(window.dbRef(window.db, 'eventConfig/dice'), (snapshot) => {
        ...
        if (data) {
            DICE_CONFIG = data; // Cập nhật biến global
...
rollBtn.addEventListener('click', () => {
    if (!isEventOpen()) {
        alert(`Sự kiện đã kết thúc! Vui lòng quay lại vào ${DICE_CONFIG.startHour}:00 Thứ 7 hàng tuần.`);
        return;
    }
    ...
    saveBtn.addEventListener('click', () => {
        ...
        window.dbSet(window.dbRef(window.db, 'eventConfig/dice'), newConfig)
            .then(() => {
                ...
                DICE_CONFIG = newConfig; // Cập nhật biến tạm ngay lập tức
...
function updateStatusUI() {
    ...
    if (isEventOpen()) {
        ...
        statusEl.innerText = `🔴 SỰ KIỆN ĐANG KHÓA (Mở vào ${DICE_CONFIG.startHour}:00 ${openDay})`;
        statusEl.style.background = "#f79290";
    }
}
